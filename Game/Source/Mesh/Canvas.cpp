	...
	for (int i = 0; i < MAPSIZE; i++)
	{
		...
		if ((i / NUM_COLUMNS) % 2 == 1)
		{
			...
			if (Tile_Type(TileSelecter) != Town_Null_Wall_ && Tile_Type(TileSelecter) != Town_Null_Door_)
			{
				...
				vec2 aVOffset = vec2((aframe.origin.x / m_MyResourceManager->GetTextureSize(0).x), (aframe.origin.y / m_MyResourceManager->GetTextureSize(0).y));
				vec2 aUVScale = vec2((aframe.size.x / m_MyResourceManager->GetTextureSize(0).x), (aframe.size.y / m_MyResourceManager->GetTextureSize(0).y));
		...
		else
		{
			...
			if (Tile_Type(TileSelecter) != Town_Null_Wall_ && Tile_Type(TileSelecter) != Town_Null_Door_)
			{
				...
				//set the UV's of the tile at the index
				vec2 aUVOffset = vec2((aframe.origin.x / m_MyResourceManager->GetTextureSize(0).x), (aframe.origin.y / m_MyResourceManager->GetTextureSize(0).y));
				vec2 aUVScale = vec2((aframe.size.x / m_MyResourceManager->GetTextureSize(0).x), (aframe.size.y / m_MyResourceManager->GetTextureSize(0).y));
	...
	for (int i = 0; i < LABSIZE; i++)
	{
		...
		if ((i / NUM_LAB_COLUMNS) % 2 == 1)
		{
			...
			if (OakLab_Tile_Type(TileSelecter) != Oak_Lab_NullWall_ && OakLab_Tile_Type(TileSelecter) != Oak_Lab_NullDoor_)
			{
				...
				//set the UV's of the tile at the index
				vec2 aVOffset = vec2((aframe.origin.x / m_MyResourceManager->GetTextureSize(1).x), (aframe.origin.y / m_MyResourceManager->GetTextureSize(1).y));
				vec2 aUVScale = vec2((aframe.size.x / m_MyResourceManager->GetTextureSize(1).x), (aframe.size.y / m_MyResourceManager->GetTextureSize(1).y));
		...
		else
		{
			...
			if (OakLab_Tile_Type(TileSelecter) != Oak_Lab_NullWall_ && OakLab_Tile_Type(TileSelecter) != Oak_Lab_NullDoor_)
			{
				...
				//set the UV's of the tile at the index
				vec2 aUVOffset = vec2((aframe.origin.x / m_MyResourceManager->GetTextureSize(1).x), (aframe.origin.y / m_MyResourceManager->GetTextureSize(1).y));
				vec2 aUVScale = vec2((aframe.size.x / m_MyResourceManager->GetTextureSize(1).x), (aframe.size.y / m_MyResourceManager->GetTextureSize(1).y));
	...
	for (int i = 0; i < FORESTMAPSIZE; i++)
	{
		...
		if ((i / NUM_FOREST_COLUMNS) % 2 == 1)
		{
			...
			if (Forest_Tile_Type(TileSelecter) != Forest_Null_Wall && Forest_Tile_Type(TileSelecter) != Forest_Null_Door)
			{
				...
				//set the UV's of the tile at the index
				vec2 aVOffset = vec2((aframe.origin.x / m_MyResourceManager->GetTextureSize(3).x), (aframe.origin.y / m_MyResourceManager->GetTextureSize(3).y));
				vec2 aUVScale = vec2((aframe.size.x / m_MyResourceManager->GetTextureSize(3).x), (aframe.size.y / m_MyResourceManager->GetTextureSize(3).y));
		...
		else
		{
			...
			if (Forest_Tile_Type(TileSelecter) != Forest_Null_Wall && Forest_Tile_Type(TileSelecter) != Forest_Null_Door)
			{
				...
				//set the UV's of the tile at the index
				vec2 aUVOffset = vec2((aframe.origin.x / m_MyResourceManager->GetTextureSize(3).x), (aframe.origin.y / m_MyResourceManager->GetTextureSize(3).y));
				vec2 aUVScale = vec2((aframe.size.x / m_MyResourceManager->GetTextureSize(3).x), (aframe.size.y / m_MyResourceManager->GetTextureSize(3).y));
